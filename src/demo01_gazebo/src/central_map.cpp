...
void central_map::dilation()
{
  ...
  for (auto &c : clusters)
  {
    ...
    std::vector<std::vector<double>> pathPoints = splitPath(c.now_point,c.next_flag, numPoints);
  ...
  for(int j=0;j<clusters.size();j++){
    for (int i = 0; i < clusters.size(); i++)
    {
      clusters[i].next_target=paths[i][j];
      // 依次执行移动函数，到达目标点
    }
    move_astep();
  }
}
void central_map::move_astep(){
  int match_num = 0;
  ...
  while (match_num < clusters.size())
  {
    ...
    for (auto &c : clusters)
    {
      if (c.now_point[0] - c.next_target[0] < 0.01)
      {
        c.vel_x = 0;
      ...
      if (c.now_point[1] - c.next_target[1] < 0.01)
      {
        c.vel_y = 0;
